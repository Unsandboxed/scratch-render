...
class ParticleSkin extends Skin {
    ...
    * Create a skin specialized for high-volume particle rendering.
    ...
    /**
     * Draw many points on the pen layer in one call.
     * @param {PenAttributes} penAttributes - how points should be drawn.
     * @param {Float32Array|Array<number>} points - interleaved x/y pairs: [x0, y0, x1, y1, ...].
     * @param {number=} count - optional number of points to draw.
     */
    drawPoints (penAttributes, points, count) {
        if (!points) return;

        const maxFromArray = Math.floor(points.length / 2);
        const pointsToDraw = typeof count === 'number' ? Math.min(count, maxFromArray) : maxFromArray;
